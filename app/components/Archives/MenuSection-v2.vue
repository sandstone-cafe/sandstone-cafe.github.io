 ...
 * ----------------
 * Fixed version:
 * - Uses AccordionRoot instead of Accordion
 * - Proper Reka-UI imports
 * - Lucide ChevronDown icon for visual affordance
      ...
      <div class="grid lg:grid-cols-[1fr_auto] gap-12 lg:gap-16">
        ...
        <div>
          <AccordionRoot type="single" collapsible default-value="Breakfast" class="space-y-3">
            <AccordionItem v-for="cat in menuCategories" :key="cat.title" :value="cat.title"
              class="border border-border rounded-xl px-6 overflow-hidden data-[state=open]:bg-card">
              <AccordionTrigger
                class="flex items-center justify-between gap-4 font-serif text-xl md:text-2xl text-foreground hover:no-underline py-5">
                <span>{{ cat.title }}</span>
                <!-- Chevron icon with rotation on open -->
                <ChevronDown
                  class="w-4 h-4 text-muted-foreground transition-transform duration-200 data-[state=open]:rotate-180" />
              </AccordionTrigger>
