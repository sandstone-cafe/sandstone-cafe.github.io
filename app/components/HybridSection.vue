    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><!-- SectionContainer -->
      ...
      <div class="grid lg:grid-cols-[1fr_auto] gap-12 lg:gap-16"><!-- Body -->
        ...
        <div><!-- Div -->
          ...
          <AccordionRoot type="single" collapsible default-value="electronics" class="space-y-3">
...
<AccordionTrigger
  class="group flex w-full items-center justify-between px-6 py-5 text-left font-semibold text-gray-900 hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 data-[state=open]:bg-gray-50">


 -->

                  <!-- <AccordionTrigger class="group flex w-full items-center justify-between px-6 py-5 "> -->
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg"
                    class="h-5 w-5 text-gray-500 transition-transform duration-500 ease-out group-data-[state=open]:rotate-180"
                  ...
                  <Icon name="radix-icons:chevron-down"
                    class=" text-gray-500 transition-transform duration-500 ease-out group-data-[state=open]:rotate-180 text-3xl" />
                  <!-- </span> -->
                </AccordionTrigger>
              </AccordionHeader>

              <!-- Content with professional smooth animation -->

              <AccordionContent
                class="accordion-content overflow-hidden transition-transform duration-500 ease-out pb-6 text-gray-600">
                <div class="space-y-4 pb-2">
                  <div v-for="item in cat.items" :key="item.name" class="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p class="text-sm text-muted-foreground font-sans text-gray-900">
                        {{ item.desc }}
                      </p>
                    </div>
              ...
              <!-- 
              <AccordionContent class="accordion-content overflow-hidden px-6 pb-6 text-gray-600">
                <ul class="space-y-2.5 pl-1 pt-1">
                  <li v-for="(item, i) in cat.items" :key="i"
                    class="flex items-center gap-2 text-sm before:h-1.5 before:w-1.5 before:rounded-full before:bg-gray-300">
                    {{ item }}
                  </li>
                </ul>
              </AccordionContent>
